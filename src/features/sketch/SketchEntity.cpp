...
std::vector<TopoDS_Edge> SketchPolyline::toEdges3D(const gp_Pln& plane) const {
    ...
    // Build a map: startIdx → FilletArc, so we can jump over arc segments
    std::map<int, const FilletArc*> arcByStart;
    for (const auto& fa : m_filletArcs)
        arcByStart[fa.startIdx] = &fa;

    // Also mark which indices are interior arc-approximation points to skip
    std::set<int> skipSegStart;
    for (const auto& fa : m_filletArcs)
        for (int k = fa.startIdx; k < fa.endIdx; ++k)
            skipSegStart.insert(k);

    std::vector<TopoDS_Edge> edges;
    int n = (int)m_points.size();
    int i = 0;
    while (i + 1 < n) {
        // Check if a fillet arc starts here
        auto it = arcByStart.find(i);
        if (it != arcByStart.end()) {
            const FilletArc& fa = *it->second;
            // T1, midpoint of arc, T2
            gp_Pnt T1  = toGlobal(m_points[fa.startIdx]);
            gp_Pnt mid = toGlobal(gp_Pnt2d(
                fa.cx + fa.radius * std::cos(fa.startAngle + fa.angleDiff * 0.5),
                fa.cy + fa.radius * std::sin(fa.startAngle + fa.angleDiff * 0.5)));
            gp_Pnt T2  = toGlobal(m_points[fa.endIdx]);

            if (T1.Distance(T2) > 1e-6 && T1.Distance(mid) > 1e-6) {
                try {
                    GC_MakeArcOfCircle arcMaker(T1, mid, T2);
                    if (arcMaker.IsDone()) {
            ...
            i = fa.endIdx; // skip all arc-approximation segments
            continue;
        }

        // Skip if this segment is part of a fillet arc interior
        if (skipSegStart.count(i)) { i++; continue; }

        // Normal straight segment
