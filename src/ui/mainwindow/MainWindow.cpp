...
#ifdef Q_OS_WIN
    m_sketchToolbar->setIconSize(QSize(20, 20));
    m_sketchToolbar->setStyleSheet("QToolButton { padding: 2px; }");
#else
        ...
        if (type == "Sketch") {
            ...
            for (const auto& eVal : entities) {
                ...
                if (eType == "Line") {
                    ...
                    for (const auto& aVal : eObj["filletArcs"].toArray()) {
                        QJsonObject aObj = aVal.toObject();
                        CADEngine::SketchPolyline::FilletArc fa;
                        fa.startIdx   = aObj["startIdx"].toInt();
                        fa.endIdx     = aObj["endIdx"].toInt();
                        fa.cx         = aObj["cx"].toDouble();
                        fa.cy         = aObj["cy"].toDouble();
                        fa.radius     = aObj["radius"].toDouble();
                        fa.startAngle = aObj["startAngle"].toDouble();
                        fa.angleDiff  = aObj["angleDiff"].toDouble();
                        p->addFilletArc(fa);
                    }
            ...
            if (sketchFeat && sketchFeat->getSketch2D()) {
                ...
                for (const auto& entity : sketchFeat->getSketch2D()->getEntities()) {
                    ...
                    if (entity->getType() == CADEngine::SketchEntityType::Line) {
                        ...
                        eObj["points"] = pts;
                        QJsonArray arcs;
                        for (const auto& arc : p->getFilletArcs()) {
                            QJsonObject aObj;
                            aObj["startIdx"]   = arc.startIdx;
                            aObj["endIdx"]     = arc.endIdx;
                            aObj["cx"]         = arc.cx;
                            aObj["cy"]         = arc.cy;
                            aObj["radius"]     = arc.radius;
                            aObj["startAngle"] = arc.startAngle;
                            aObj["angleDiff"]  = arc.angleDiff;
                            arcs.append(aObj);
                        }
                        if (!arcs.isEmpty()) eObj["filletArcs"] = arcs;
                    }
...
void MainWindow::onFilletVertex(std::shared_ptr<CADEngine::SketchPolyline> polyline, int vertexIndex) {
    ...
    // Net: remove V (1 point), insert T1+T2 (2 points) → net +1 at vertexIndex
    // Shift existing fillet arc indices that come after this vertex
    polyline->shiftFilletArcs(vertexIndex + 1, 1);

    // Store arc data: T1 at vertexIndex, T2 at vertexIndex+1
    CADEngine::SketchPolyline::FilletArc fa;
    fa.startIdx   = vertexIndex;
    fa.endIdx     = vertexIndex + 1;
    fa.cx         = center.X();
    fa.cy         = center.Y();
    fa.radius     = radius;
    fa.startAngle = startAngle;
    fa.angleDiff  = angleDiff;
    polyline->addFilletArc(fa);

    // Replace V with T1 and T2 only (arc rendered from FilletArc data in viewport)
...
void MainWindow::onFilletRectCorner(std::shared_ptr<CADEngine::SketchRectangle> rect, int cornerIdx)
{
    ...
    // Arc de congé : vrai arc circulaire (pas Bézier)
    auto arc = std::make_shared<CADEngine::SketchArc>(T1, T2, arcMid, false);
    ...
    std::shared_ptr<CADEngine::SketchLine> line2, bool line2AtStart)
{
    ...
    // Arc de congé : vrai arc circulaire (arcMid est sur le cercle, pas un point de contrôle Bézier)
    // Un arc circulaire garantit une seule face cylindrique dans un balayage (pas de tessellation)
    auto arc = std::make_shared<CADEngine::SketchArc>(T1, T2, arcMid, false);
    sketch2D->addEntity(arc);
        ...
        if (entity->getType() == CADEngine::SketchEntityType::Line) {
            ...
            if (poly) {
                for (const auto& pt : poly->getPoints())
                    expandBBox(pt.X(), pt.Y());
                // Include arc bulge midpoints in bbox
                for (const auto& fa : poly->getFilletArcs()) {
                    double amid = fa.startAngle + fa.angleDiff * 0.5;
                    expandBBox(fa.cx + fa.radius * std::cos(amid),
                               fa.cy + fa.radius * std::sin(amid));
                }
            }
        }
        else if (entity->getType() == CADEngine::SketchEntityType::Ellipse) {
    ...
    for (const auto& entity : sketch2D->getEntities()) {
        ...
        // --- POLYLINE ---
            ...
            if (polyline) {
                ...
                auto findArc = [&](int idx) -> const CADEngine::SketchPolyline::FilletArc* {
                    for (const auto& fa : filletArcs) if (fa.startIdx == idx) return &fa;
                    return nullptr;
                };
                if (filletArcs.empty()) {
                    QVector<QPointF> pts;
                    for (const auto& pt : points) pts << toPage(pt.X(), pt.Y());
                    painter.drawPolyline(pts.data(), pts.size());
                } else {
                    int np = (int)points.size();
                    int idx = 0;
                    while (idx < np) {
                        if (const auto* fa = findArc(idx)) {
                            int endIdx = (fa->endIdx < np) ? fa->endIdx : np - 1;
                            QVector<QPointF> arcPts;
                            for (int k = 0; k <= 32; k++) {
                                double a = fa->startAngle + (double)k / 32 * fa->angleDiff;
                                arcPts << toPage(fa->cx + fa->radius * std::cos(a),
                                                 fa->cy + fa->radius * std::sin(a));
                            }
                            painter.drawPolyline(arcPts.data(), arcPts.size());
                            idx = endIdx;
                        } else {
                            if (idx + 1 < np)
                                painter.drawLine(toPage(points[idx].X(), points[idx].Y()),
                                                 toPage(points[idx+1].X(), points[idx+1].Y()));
                            idx++;
                        }
                    }
                }
...
void MainWindow::onExportSketchPlan(std::shared_ptr<CADEngine::SketchFeature> sketch) {
    ...
    for (const auto& entity : sketch2D->getEntities()) {
        ...
        if (entity->getType() == CADEngine::SketchEntityType::Line) {
            ...
            if (pl) {
                const auto& points = pl->getPoints();
                const auto& filletArcs = pl->getFilletArcs();
                auto findArc = [&](int idx) -> const CADEngine::SketchPolyline::FilletArc* {
                    for (const auto& fa : filletArcs) if (fa.startIdx == idx) return &fa;
                    return nullptr;
                };
                if (filletArcs.empty()) {
                    QVector<QPointF> pts;
                    for (const auto& p : points) pts << toPage(p.X(), p.Y());
                    painter.drawPolyline(pts.data(), pts.size());
                } else {
                    int np = (int)points.size();
                    int idx = 0;
            ...
            if (ellipse) {
                ...
                for (int i = 0; i <= 64; ++i) {
                    ...
                    double lx = a * std::cos(t), ly = b * std::sin(t);
                    pts << toPage(cx + lx*cosR - ly*sinR, cy + lx*sinR + ly*cosR);
                }
                painter.drawPolyline(pts.data(), pts.size());
                QPointF c = toPage(cx, cy);
                painter.setPen(centerPen);
                painter.drawLine(QPointF(c.x()-crossSz,c.y()), QPointF(c.x()+crossSz,c.y()));
                painter.drawLine(QPointF(c.x(),c.y()-crossSz), QPointF(c.x(),c.y()+crossSz));
                painter.setPen(entityPen);
            ...
            if (polygon) {
                auto verts = polygon->getVertices();
                QPolygonF poly;
                for (const auto& v : verts) poly << toPage(v.X(), v.Y());
                poly << poly.first();
                painter.drawPolygon(poly);
                QPointF c = toPage(polygon->getCenter().X(), polygon->getCenter().Y());
                painter.setPen(centerPen);
            ...
            if (oblong) {
                double cx = oblong->getCenter().X(), cy = oblong->getCenter().Y();
                double L = oblong->getLength(), W = oblong->getWidth();
                double rot = oblong->getRotation(), r = W / 2.0, halfS = (L - W) / 2.0;
                double cosR = std::cos(rot), sinR = std::sin(rot);
                double axX = cosR, axY = sinR, prX = -sinR, prY = cosR;
                QVector<QPointF> pts;
                pts << toPage(cx - halfS*axX + r*prX, cy - halfS*axY + r*prY);
                pts << toPage(cx + halfS*axX + r*prX, cy + halfS*axY + r*prY);
                double c2x = cx + halfS*axX, c2y = cy + halfS*axY;
                double sa = std::atan2(prY, prX);
                for (int i = 1; i <= 32; ++i) { double a = sa - M_PI*i/32; pts << toPage(c2x+r*std::cos(a), c2y+r*std::sin(a)); }
                pts << toPage(cx - halfS*axX - r*prX, cy - halfS*axY - r*prY);
                double c1x = cx - halfS*axX, c1y = cy - halfS*axY;
                for (int i = 1; i <= 32; ++i) { double a = sa+M_PI-M_PI*i/32; pts << toPage(c1x+r*std::cos(a), c1y+r*std::sin(a)); }
                painter.drawPolygon(pts.data(), pts.size());
                QPointF c = toPage(cx, cy);
                painter.setPen(centerPen);
                painter.drawLine(QPointF(c.x()-crossSz,c.y()), QPointF(c.x()+crossSz,c.y()));
