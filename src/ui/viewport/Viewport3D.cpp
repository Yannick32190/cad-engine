...
void Viewport3D::resizeGL(int w, int h) {
    // Utiliser la vraie taille du widget
    int actualW = width();
    int actualH = height();
    ...
    if (m_mode == ViewMode::SKETCH_2D) {
        // Projection orthogonale pour sketch
        double aspect = (double)actualW / (double)actualH;
        double size = m_sketch2DZoom;  // Taille visible - ZOOMABLE !
        glOrtho(-size * aspect, size * aspect, -size, size, -1000.0, 1000.0);
    } else {
        // Projection perspective pour 3D
        double aspect = (double)actualW / (double)actualH;
...
void Viewport3D::paintGL() {
    ...
    if (!m_textLabels.empty() || !m_axisLabels.empty() || !m_axisTickLabels.empty()) {
        ...
        for (const auto& label : m_textLabels) {
            ...
            // OpenGL a origine en bas, Qt en haut
            int screenY = height() - static_cast<int>(winY);
            ...
            // Dessiner texte centré (avec fond semi-transparent pour lisibilité)
            QRect textRect(screenX - textW/2, screenY - textH/2, textW, textH);
        ...
        if (!m_axisTickLabels.empty()) {
            ...
            for (const auto& tick : m_axisTickLabels) {
                ...
                int sx = static_cast<int>(winX);
                int sy = height() - static_cast<int>(winY);
    ...
    for (const auto& entity : sketch2D->getEntities()) {
        ...
        if (entity->getType() == CADEngine::SketchEntityType::Line) {
            ...
            if (polyline) {
                ...
                // Dessiner les VERTICES comme des petits carrés (pour montrer qu'ils sont draggable)
                glPointSize(7.0f);
                glColor3f(0.2f, 0.2f, 0.8f);  // Bleu foncé
                glBegin(GL_POINTS);
                for (const auto& pt : points) {
                    glVertex3f(pt.X(), pt.Y(), 0.15f);
                }
                glEnd();
                glColor3f(0.0f, 0.0f, 0.0f);  // Retour noir
            }
        }
...
gp_Pnt Viewport3D::screenTo3DRay(int screenX, int screenY, gp_Dir& rayDir) {
    ...
    double winY = viewport[3] - screenY;
    
    GLdouble x1, y1, z1, x2, y2, z2;
    gluUnProject(screenX, winY, 0.0, modelview, projection, viewport, &x1, &y1, &z1);
    gluUnProject(screenX, winY, 1.0, modelview, projection, viewport, &x2, &y2, &z2);
    ...
    for (; explorer.More(); explorer.Next()) {
        ...
        try {
            ...
            for (int i = 0; i <= nSamples; i++) {
                ...
                double dist = std::sqrt((sx - screenX) * (sx - screenX) + (sy - screenY) * (sy - screenY));
                if (dist < bestDist) {
                    bestDist = dist;
                    bestEdge = edge;
                }
